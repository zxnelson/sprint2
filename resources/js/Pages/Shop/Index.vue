<template>
    <app-layout :title="` ${ categoryName }`">
        <secondary-header>
            <template #breadcrumbs>
                <icon name="angle-right" class="w-4 h-4 fill-current"></icon>
                <span>Tienda {{ categoryName }}</span>
            </template>
            <template #search>
        ...
        <div class="flex">
            ...
            <div class="border-l w-4/5">
                <div class="flex justify-end space-x-2 pt-4 pr-4" v-if="categorySlug">
                    ...
                    <Link :href="route('shop.index', { category: categorySlug, sort: 'low_high'})" class="hover:text-yellow-500">De Menor a Mayor</Link>
                    <span>|</span>
                    <Link :href="route('shop.index', { category: categorySlug, sort: 'high_low'})" class="hover:text-yellow-500">De Mayor a Menor</Link>
                </div>
